...
export default function ConnectionsPage() {
    ...
    async function getList() {
        if (list.length == 0) {
            ...
            await API.get(`/service/getServices?accessToken=${global.accessToken}`)
    ...
    return(
        <ImageBackground source={require('../assets/background.png')} style={{position: 'absolute', left: 0, top: 0, width: Dimensions.get('window').width, height: Dimensions.get('window').height+25}}>
            ...
            <FlatList data={list} style={tailwind('pt-8')} renderItem={({item}) => <ServiceConnection item={item} />} keyExtractor={item => item.name}/>
        </ImageBackground>
    )
}
